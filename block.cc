...
bool Block::canMove(Board *board, std::vector<int> dir) {
  ...
  for (auto newTile : tempTiles_) {
    ...
    if (board->getTileAt(newTile->getXCoordinate(), newTile->getYCoordinate())
            ->isFilled()) {
      for (auto tile : tiles_) {
        if (newTile->getXCoordinate() == tile->getXCoordinate() &&
            newTile->getYCoordinate() == tile->getYCoordinate()) {
          // There's a tile in the current block that exists at the same
          // location
...
bool Block::canMoveLeft(Board *board) { return canMove(board, {-1, 0}); }

bool Block::canMoveRight(Board *board) { return canMove(board, {1, 0}); }

bool Block::canMoveDown(Board *board) { return canMove(board, {0, 1}); }
...
void Block::moveLeft(Board *board) {
  ...
  for (auto tile : tiles_) {
    tile->reset();
    tempTiles_.push_back(
        board->getTileAt(tile->getXCoordinate() - 1, tile->getYCoordinate()));
  }
...
void Block::moveDown(Board *board) {
  ...
  for (auto tile : tiles_) {
    tile->reset();
    tempTiles_.push_back(
        board->getTileAt(tile->getXCoordinate(), tile->getYCoordinate() + 1));
  }
...
void Block::rotateClockwise(Board *board) {
  ...
  // Find the new tiles
  for (auto tile : tiles_) {
    int newX = tile->getXCoordinate() -
               (tile->getXCoordinate() - endPoints[0]) +
               (endPoints[3] - tile->getYCoordinate());
    int newY = tile->getYCoordinate() +
               (tile->getXCoordinate() - endPoints[1]) +
               (endPoints[3] - tile->getYCoordinate());
    tempTiles_.push_back(board->getTileAt(newX, newY));
  }
...
void Block::rotateCounterClockwise(Board *board) {
  ...
  if (!canRotateCounterClockwise(board)) {
    return;
  }
  ...
  for (auto tile : tiles_) {
    int newX = tile->getXCoordinate() -
               (tile->getXCoordinate() - endPoints[0]) -
               (endPoints[2] - tile->getYCoordinate());
    int newY = tile->getYCoordinate() -
               (tile->getXCoordinate() - endPoints[0]) +
               (endPoints[3] - tile->getYCoordinate());
    tempTiles_.push_back(board->getTileAt(newX, newY));
...
int Block::getNumTiles() { return tiles_.size(); }

int Block::getGenLevel() { return levelGen_; }

bool Block::isHeavy() { return isHeavy_; }
...
void Block::moveTileDown(Tile *oldTile, Board *board) {
  ...
  Tile *tileBelow = board->getTileAt(oldTile->getXCoordinate(),
                                     oldTile->getYCoordinate() + 1);
...
void Block::resetToLocations(Board *board,
  ...
  for (auto tile : tiles_) {
    tile->reset();
    tempTiles_.push_back(
        board->getTileAt(locations[index][0], locations[index][1]));
    index++;
  }
